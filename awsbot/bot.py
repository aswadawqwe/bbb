...
'''
Bots = [nadyaMID,kiMID]
creator = ["u7f5ad95e87dc9127edf0c62073e73385"]
Owner = ["u7f5ad95e87dc9127edf0c62073e73385"]
admin = ["u7f5ad95e87dc9127edf0c62073e73385"]
...
def helpmessage():
    helpMessage = "╔═══════════════" + "\n" + \
                  ...
                  "╠➥ JadwalShalat" + "\n" + \
                  "╠➥ Kalender" + "\n" + \
                  "╠➥ InstagramInfo「wendyamarul」" + "\n" + \
                  "╠➥ InstagramPost「UserName」" + "\n" + \
                  "╠➥ SearchYoutube「Metallica」" + "\n" + \
                  "╠➥ SearchImage「Bunga Mawar」" + "\n" + \
                  "╠➥ ScreenshootWebsite「www.google.com」" + "\n" + \
                  "║" + "\n" + \
                  "╠✪〘 Owner Only 〙✪═════════" + "\n" + \
                  "╠➥ AdminAdd" + "\n" + \
                  "╠➥ AdminDel" + "\n" + \
                  ...
                  "║" + "\n" + \
                  "╚═〘 Owner : Wendy Amarul  〙" + "\n" + \
                  "Level : 1.Owner - 2.Admin - 3.Member(Coming Soon) "
    return helpMessage
...
def helptexttospeech():
    helpTextToSpeech =   "╔══〘 T E X T   T O   S P E E C H 〙" + "\n" + \
                         ...
                         "╠ cy : Welsh" + "\n" + \
                         "╚══〘 Coded By : Wendy Amarul 〙" + "\n" + "\n\n" + \
                          "Contoh : say-id Nama Saya Wendy Amarul"
...
def helptranslate():
    helpTranslate =    "╔══〘 T R A N S L A T E 〙" + "\n" + \
                       ...
                       "╠ he : Hebrew" + "\n" + \
                       "╚══〘 Jangan Typo 〙" + "\n" + "\n\n" + \
                         "Contoh : en-id My Name Is Wendy Amarul"
    ...
    try:
        ...
        if op.type == 5:
            ...
            if settings["autoAdd"] == True:
                nadya.sendMessage(op.param1, "Hallo {} terimakasih telah menambahkan saya sebagai teman".format(str(nadya.getContact(op.param1).displayName)))
        if op.type == 13:
...
#==============================================================================#
                if text.lower() == 'help':
                    ...
                    nadya.sendContact(to, "u7f5ad95e87dc9127edf0c62073e73385")
...
#==============================================================================#
                    ...
                    if msg.toType == 2:
                        ...
                        if group.preventedJoinByTicket == False:
                            ...
                            nadya.sendMessage(to, "Grup QR tidak terbuka silahkan buka terlebih dahulu dengan perintah {} GroupTicket on".format(str(settings["keyCommand"])))
...
#-------------------------------------------------------------------------------
                    ...
                    if msg._from in Owner:
                        if msg.toType == 2:
                            ...
                            nadya.sendMessage(msg.to,"「 Sorry guys :v」")
                            targets = []
                            for g in gs.members:
...
#==============================================================================#
                    ...
                    if msg.to in read['readPoint']:
                            ...
                            with open('read.json', 'w') as fp:
                                ...
                                nadya.sendMessage(msg.to,"SetRead already on")
                    else:
                        try:
                            del read['readPoint'][msg.to]
                            del read['readMember'][msg.to]
                    ...
                    if msg.to not in read['readPoint']:
                        nadya.sendMessage(msg.to,"SetRead already off")
                    else:
                        try:
                            del read['readPoint'][msg.to]
                            del read['readMember'][msg.to]
                    ...
                    if msg.to in read["readPoint"]:
                        ...
                        nadya.sendMessage(msg.to, "SetRead belum diaktifin gblk ngapain di reset?")
                    ...
                    if receiver in read['readPoint']:
                        if read["ROM"][receiver].items() == []:
                            nadya.sendMessage(receiver,"[ Sider ]:\nNone")
                        else:
                            chiya = []
                        ...
                        nadya.sendMessage(receiver,"SetRead has not been set.")
...
#==============================================================================#
                    ...
                    with requests.session() as web:
                        r = web.get("http://image.thum.io/get/http://{}/".format(urllib.parse.quote(query)))
                        data = r.text
                    ...
                    with requests.session() as web:
                        ...
                        r = web.get("https://rest.farzain.com/api/ig_profile.php?id={}&apikey=OxZznc3su0h9AbipXorpjTQP8".format(search))
                        try:
                    ...
                    with requests.session() as web:
                        web.headers["User-Agent"] = random.choice(settings["userAgent"])
                        r = web.get("https://rest.farzain.com/api/gambarg.php?id={}&apikey=OxZznc3su0h9AbipXorpjTQP8".format(urllib.parse.quote(search)))
# (Alternative API) https://www.googleapis.com/customsearch/v1?key=AIzaSyANNivkhi50orHOST1x_S-q9cfGnSv8mew&cx=017321112510921362281:5jabph47iso&siteSearch=images.google.com/?gws_rd=ssl&searchType=image&fileType=jpg&imgSize=xlarge&q={}&alt=json
                        data = r.text
                        data = json.loads(data)
                        if data["result"] != []:
                            items = data["result"]
                    ...
                    with requests.session() as web:
                        ...
                        nadya.sendMessage(to, str(ret_))
                            ...
                            try:
                                ...
                                r = requests.get("https://farzain.com/api/joox.php?id={}".format(urllib.parse.quote(search)))
                                data = r.text
                                data = json.loads(data)
                                info = data["info"]
                                audio = data["audio"]
...
#===============================================================================[NEW]
                ...
                with requests.session() as web:
                    web.headers["user-agent"] = random.choice(settings["userAgent"])
                    r = web.get("https://rest.farzain.com/api/shalat.php?id={}&apikey=OxZznc3su0h9AbipXorpjTQP8".format(urllib.parse.quote(location)))
                    data = r.text
                    data = json.loads(data)
            ...
            elif msg.text.lower().startswith("checkcuaca "):
                sep = text.split(" ")
                location = text.replace(sep[0] + " ","")
                with requests.session() as web:
                    web.headers["user-agent"] = random.choice(settings["userAgent"])
                    r = web.get("https://rest.farzain.com/api/cuaca.php?id={}&apikey=OxZznc3su0h9AbipXorpjTQP8".format(urllib.parse.quote(location)))
                    data = r.text
                    data = json.loads(data)
                    if "result" not in data:
                        ret_ = "╔══[ Status Cuaca ]"
                        ret_ += "\n╠ Lokasi : " + data[0].replace("Temperatur ","")
                        ret_ += "\n╠ Cuaca : " + data[1].replace("Cuaca ","")
                        ret_ += "\n╠ Deskripsi : " + data[2].replace("Deskripsi ","")
                        ret_ += "\n╠ Suhu : " + data[3].replace("Suhu : ","")
                        ret_ += "\n╠ Kelembaban : " + data[4].replace("Kelembaban : ","")
                        ret_ += "\n╠ Tekanan Udara : " + data[5].replace("Tekanan udara : ","")
                        ret_ += "\n╠ Kecepatan Angin : " + data[6].replace("Kecepatan angin : ","")
                        ret_ += "\n╚══[ Complete ]"
                    else:
                        ret_ = "[ Statu Cuaca ] Error : Lokasi tidak ditemukan"
                        nadya.sendMessage(to, str(ret_))

            elif msg.text.lower().startswith("checklokasi "):
                sep = text.split(" ")
                location = text.replace(sep[0] + " ","")
                with requests.session() as web:
                    web.headers["user-agent"] = random.choice(settings["userAgent"])
                    r = web.get("https://rest.farzain.com/api/lokasi.php?id={}}&apikey=OxZznc3su0h9AbipXorpjTQP8".format(urllib.parse.quote(location)))
                    data = r.text
                    data = json.loads(data)
                ...
                nadya.sendMessage(to, "Goblok cek sendiri lah jangan manja")
